...
<form action="index.php?controller=artists&action=<?= isset($artist) ||  (isset($_SESSION['old_inputs']) && $_GET['action'] == 'edit') ? 'edit&id='. $_GET['id'] : 'add' ?>" method="post" enctype="multipart/form-data">
	...
	<select name="label_id[]" id="label_id" multiple>
		...
		<?php foreach($labels as $label): ?>
            <?php
                $selected = false;
                foreach($artistLabels as $artistLabel){
                    if($label['id'] == $artistLabel['id']){
                        $selected=true;
                    }
                    //$label['id'] !=$artistLabel['id']? :$selected=true;
                }
            ?>

			<option value="<?= $label['id']; ?>" <?php if(isset($artist)&& $selected==true): ?>selected="selected"<?php endif; ?>><?= $label['name']; ?></option>
		<?php endforeach; ?>
